...
<!-- 	</script>
	...
	function validatString(event) {
		const input = event.target;
		input.value = input.value.replace(^[^-\\s][\\p{L} .']+$, '');
		if(input.value.length > 0 && input.value[0] <= '0') {
	...
	<div>
		<form class="pb-5" action="<%=ORSView.OWNER_LIST_CTL%>" method="post">
			...
			<div class="row">
				...
				<div class="col-sm-2">
					<input type="text" name="insurenceAmount"
						placeholder="Enter Amount" class="form-control"
						oninput="validateMobileNo(event)" maxlength="10"
						value="<%=ServletUtility.getParameter("insurenceAmount", request)%>">
				</div>
